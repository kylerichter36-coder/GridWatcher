...
"""
            ...
            git_targets = ["home_sender/grid_model.h", "version.json", "windows_trainer/telemetry.csv"]
            if os.path.exists(os.path.join(REPO_DIR, "home_sender.bin")): git_targets.append("home_sender.bin")
            if os.path.exists(os.path.join(REPO_DIR, "handheld.bin")): git_targets.append("handheld.bin")
            
            run_git(["git", "add"] + git_targets)
