...
class RetrainerWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Read current version to embed in header (version bump happens in step 4)
            cur_version = 1
            if os.path.exists(VERSION_JSON_PATH):
                try:
                    with open(VERSION_JSON_PATH, "r") as vf:
                        cur_version = json.load(vf).get("version", 1) + 1
                except: pass
...
#define ML_MODEL_VERSION {cur_version}
#endif
...
"""
            ...
            with open(MODEL_HEADER_PATH, "w") as f:
                f.write(header_code)
            self.log_signal.emit(f"       [SUCCESS] Decision weights exported to grid_model.h (v{cur_version}).")
